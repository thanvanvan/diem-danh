...
import dash
from dash import dcc, html, dash_table, callback, Input, Output, State, ctx # Thêm ctx
...
def get_google_sheet_client():
    ...
    try:
        ...
        print("Lỗi: Không thể phân tích cú pháp chuỗi JSON từ GCP_CREDENTIALS_JSON.")
        return None
...
def generate_qr_with_token(expiry_minutes=5):
    """Tạo mã QR chứa token và thời gian hết hạn."""
    token = secrets.token_urlsafe(8)
    expiry_timestamp = int(time.time() + expiry_minutes * 60)
    
    entry_value = f"{token}::{expiry_timestamp}"
    
    # LƯU Ý QUAN TRỌNG: Bạn cần phải lấy Entry ID thực tế của câu hỏi TOKEN trong Form
    # và thay thế 'entry.123456789' bằng ID đó.
    # Cách lấy: Mở form > Nút 3 chấm > Get pre-filled link > Điền "test" vào câu hỏi TOKEN > Get Link
    # Trong link vừa lấy, tìm đoạn `entry.xxxxxxxx=` và sao chép nó.
    qr_url = f"https://docs.google.com/forms/d/e/{FORM_ID}/viewform?usp=pp_url&entry.123456789={entry_value}"
...
def load_and_process_attendance():
    """Tải và xử lý dữ liệu điểm danh từ Google Sheets."""
    client = get_google_sheet_client()
    if not client:
        return pd.DataFrame([{"Lỗi": "Không thể kết nối đến Google Sheets. Kiểm tra credentials."}])
    try:
        ...
        return pd.DataFrame([{"Lỗi": f"Không tìm thấy Spreadsheet. ID có đúng không?"}])
    except gspread.exceptions.WorksheetNotFound:
        return pd.DataFrame([{"Lỗi": f"Không tìm thấy Sheet '{SHEET_NAME}'"}])
    except Exception as e:
        return pd.DataFrame([{"Lỗi": f"Lỗi không xác định: {e}"}])

# --- PHẦN LAYOUT VÀ CALLBACKS GIỮ NGUYÊN NHƯ TRƯỚC ---
# (Không cần thay đổi gì ở phần này)
# ...
# --- 3. BỐ CỤC ỨNG DỤNG (LAYOUT) ---
